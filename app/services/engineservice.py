...
from app.dataclasses.engineservice_dataclass import CurrentModeOut
...
class EngineService:
    ...
    def send_currentmode(self, data_value):
        if not self.instance:
            logger.warning("Service not running, cannot send CurrentMode")
            return

        msg = CurrentModeOut()
        msg.from_json(data_value)
        logger.debug(f"Sending CurrentMode: {data_value}")
    ...
    async def init_service(self):
        ...
        self.instance = await construct_server_service_instance(
            service=service_def,
            instance_id=1,
            endpoint=(ipaddress.IPv4Address(INTERFACE_IP), 10128),
            ttl=255,
        ...
        logger.info(f"EngineService Server Started on port 10128")
...
async def initialize_engineservice(sd):
    service_manager = EngineService(sd)
    await service_manager.init_service()
    try:
        await asyncio.Future()
    except asyncio.CancelledError:
        logger.info("Shutting down EngineService...")
    finally:
